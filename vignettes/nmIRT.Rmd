...
scale <- scale_from_df(df, item='ITEM', dv='DV')
```
...
```{r, eval=FALSE, fig.show='hold'}
scale <- scale_from_csv("mydata.csv")
```
...
```{r, eval=FALSE}
model <- add_dataset(model, "my.dta", use_path=FALSE)
```
...
```{r, eval=FALSE}
model <- consolidate_levels_in_model(model, 23, c(4))
```
...
```{r, eval=FALSE}
count <- 5
model <- automatic_consolidation(model, count)
```
...
```{r}
inits <- initial_estimates_overview(model)
head(inits, n=12)
```
...
```{r, eval=FALSE}
model <- initial_estimates_from_data(model)
```
