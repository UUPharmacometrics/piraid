...
The nmIRT package simplifies working with item response theory (IRT) models within the non-linear mixed effect software NONMEM. The package is designed to facilitate 
...
## An IRT analysis with nmIRT

In this vignette, we illustrate some important steps during an IRT-based analysis that nmIRT can facilitate. The data for this analysis come from a simulated phase II study that measured the hypothetical rheumatoid arthritis (HRA) score. The dataset is provided with the package and can be accessed using   

```{r}
hra_data_path <- system.file("extdata","hra-score-data.csv", 
                             package = "nmIRT") # use internal hra score dataset 
```
...
The assessment itself is described within nmIRT through scale objects, which store information about the number of items, their name and the number of valid levels for each item. The scale objects can be created in a number of different ways, for our HRA scale a scale definition file is provided and we can simply load it using      
...
The resulting model object can then be further manipulated by, for example, setting a run number or adding the actual dataset. The `nmIRT` package provides the pipe operator from the `magrittr` package to simplify the chaining of manipulation operation, i.e., we can write 
...
to make these changes to our model. With a dataset associated with the model, nmIRT can also perform the estimation of the item parameters using the mirt package. This is a two step process. First, all item parameters in the model are estimated using  
...
When we are satisfied with the model, we can ask nmIRT to generate the corresponding NONMEM code, either by printing to the R console using
...
The next step in the workflow is to run the generated model in NONMEM. This has to be done outside the nmIRT packge. 
